...
FORCE_INLINE void _draw_centered_temp(const int16_t temp, const uint8_t x, const uint8_t y) {            //Tech temperature on display /edit here
  const char * const str = itostr3left(temp);
...
FORCE_INLINE void _draw_heater_status(const uint8_t x, const int8_t heater, const bool blink) {
  ...
  if (PAGE_CONTAINS(21, 28)) {
    _draw_centered_temp(0.5f + (
      ...
      ), x+8, 28                                   // x moves Tech %/temp on display
    );
  }
}
...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_UNDER(STATUS_SCREENHEIGHT + 1)) {
    ...
    u8g.drawBitmapP(
      STATUS_SCREEN_X, STATUS_SCREEN_Y,                                 //x+2 move fan logo
...
//mine2
 ...
 //Laser Power %   TYPE HERE
            /* 
           * add key in line with m01 12v+ and create 12/5v voltage divider for digital in 11 
           *            see picture in files
           *       **added temp checks for TECH(t1) and laser(t0)**
           */
     ...
     if (techTemp < 15 || techTemp > 56 || laserTemp < 15 || laserTemp > 36 ){
          ...
          else if (digitalRead(ARMED_PIN) == LOW){
              lcd_setstatus("    Laser Disarmed.    ");
              u8g.drawBitmapP(12,8, LASER_ICON_BYTEWIDTH,LASER_ICON_HEIGHT, lockicon_bmp); 
              }        
          }
   
#elif   // ARMED SWITCH option disabled
    
        if (laserPower > 0) {
          lcd_setstatus(" DANGER!Laser Firing!");
          u8g.drawBitmapP(6,4, LASERENABLE_BYTEWIDTH, LASERENABLE_HEIGHT, laserenable_bmp);  //mine2added
          u8g.drawBitmapP(LCD_LASER_BOX_X + 4, 8, LASER_ICON_BYTEWIDTH, LASER_ICON_HEIGHT, laseron_bmp);
          u8g.setPrintPos(LCD_LASER_BOX_X + 22, 9);
          u8g.print(laserPower);
          lcd_printPGM(PSTR("%"));     
...
//Laser Current
         u8g.setPrintPos(LCD_LASER_BOX_X + 22, 18);
         u8g.print("0.0");                                       // get amperage value from meter in laser box, use an analog-IN pin
         lcd_printPGM(PSTR("A")); 
