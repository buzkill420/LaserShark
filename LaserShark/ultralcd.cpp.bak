...
#if ENABLED(TECH_AUTO_COOLING)              //added for auto tech cooling
  static bool tech_auto = true;
#else 
  static bool tech_auto = false;
#endif

#if ENABLED(TECH_AUTO_COOLING)      //added for pump control
static bool air_assist = false;
#endif
...
uint16_t max_display_update_time = 0;
  ...
  void lcd_control_temperature_menu2();
...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  void lcd_main_menu() {
    ...
    if (planner.movesplanned() || IS_SD_PRINTING())
      ...
      MENU_ITEM(submenu, "Control", lcd_prepare_menu);
      MENU_ITEM(submenu, "Settings", lcd_control_menu);
    //custom commands menu moved
    #if ENABLED(CUSTOM_USER_MENUS)
  ...
  void lcd_prepare_menu() {
    ...
    MENU_BACK(MSG_MAIN);
     ...
     #if HAS_FAN1
        MENU_MULTIPLIER_ITEM_EDIT(int3, "Laser Power", &fanSpeeds[1], 0, 255);     
      #endif                                                                                                  //mine  fan1 moved from temp menu
  ...
  void lcd_control_menu() {
    ...
    MENU_ITEM(submenu, MSG_TEMPERATURE, lcd_control_temperature_menu2);
 ...
 //
   ...
   *
   */
  void lcd_control_temperature_menu() {      // for when in "Control" menu
    START_MENU();
    MENU_BACK("Control");    // ^ Control

    if(tech_auto == false) MENU_MULTIPLIER_ITEM_EDIT(int3, "Tech Power", &fanSpeeds[0], 0, 255);       //  Tech power level
    MENU_ITEM_EDIT_CALLBACK(bool , "Auto-Cooling", &tech_auto, _tech_auto);         //Tech auto cooling on/off menu item
    #if ENABLED(AIR_PUMP)
        MENU_ITEM_EDIT_CALLBACK(bool , "Air-Assist", &air_assist, _air_pump);         //Air pump on/off menu item
    #endif

    END_MENU();
  }

   void lcd_control_temperature_menu2() {         // for when in "settings" menu
    START_MENU(); 
    MENU_BACK("Settings");       // ^ Settings   
...
void _tech_auto(){                                    // resets fanspeed when enabling auto cooling.
    if (tech_auto == true) fanSpeeds[0] = 0;
}
   ...
   */
    ...
    void lcd_info_thermistors_menu() {
      ...
      STATIC_ITEM("Laser: " THERMISTOR_NAME, false, true);
      ...
      STATIC_ITEM(MSG_INFO_MAX_TEMP ": " STRINGIFY(HEATER_0_MAXTEMP), false);
        ...
        #include "thermistornames.h"
        STATIC_ITEM("Tech:" THERMISTOR_NAME, false, true);
    ...
    void lcd_info_board_menu() {
      ...
      u8g.setPrintPos(19,11); 
      u8g.print(BOARD_NAME);
      u8g.setFont(u8g_font_04b_03);  
      u8g.setPrintPos(35,26);                             
      u8g.print(MSG_INFO_BAUDRATE ": " STRINGIFY(BAUDRATE));
      u8g.setPrintPos(38,36);
      u8g.print(MSG_INFO_PROTOCOL ": " PROTOCOL_VERSION);
      u8g.setPrintPos(38,46);
      #if POWER_SUPPLY == 0
      u8g.print(MSG_INFO_PSU ": Generic");
      #elif POWER_SUPPLY == 1
      u8g.print(MSG_INFO_PSU ": ATX");
      #elif POWER_SUPPLY == 2
      u8g.print(MSG_INFO_PSU ": XBox");
      #endif
      END_SCREEN();
    ...
    void lcd_info_printer_menu() {
      if (use_click()) { return lcd_goto_previous_menu(); }
      START_SCREEN();  
      u8g.drawBitmapP(87,17,LASERSHARK_WIDTH,LASERSHARK_HEIGHT,LASERSHARK_bmp);   
      u8g.setFont(u8g_font_tpssb);
      u8g.setPrintPos(16,11);
      u8g.print("LaserShark");
      u8g.setFont(u8g_font_04b_03);  
      u8g.setPrintPos(25,18); 
      u8g.print(BUZ_VERSION);
      u8g.setPrintPos(25,26);
      u8g.print(STRING_DISTRIBUTION_DATE);
      u8g.setPrintPos(32,40);
      u8g.print("Based On:");
      u8g.setPrintPos(25,47);                              
      u8g.print("Marlin 1.1.9.1");
      u8g.setPrintPos(15,60);
      u8g.print("Compiled By BuZkill"); 
            
     /*  STATIC_ITEM(" ", true);                         
      STATIC_ITEM("Marlin", true);                                    // Marlin
   ...
   void lcd_info_laser_menu() {
      if (use_click()) { return lcd_goto_previous_menu(); }
      START_SCREEN();
      u8g.setFont(u8g_font_tpssb);
      u8g.drawBitmapP(4,4,E1_WIDTH,E1_HEIGHT,Endurance1_bmp);
      u8g.drawBitmapP(70,4,E2_WIDTH,E2_HEIGHT,Endurance2_bmp);
      u8g.setPrintPos(21,13); 
      u8g.print("NDURANC   Lasers");
      u8g.setFont(u8g_font_04b_03);  
      u8g.setPrintPos(20,26);                             
      u8g.print("Class IV  Laser Diode");
      u8g.setPrintPos(16,36);
      u8g.print("10Watt+ /w Tech Cooler");
      u8g.setPrintPos(9,46);
      u8g.print("12VDC/120W // 3.3-12v TTL ");
      u8g.setPrintPos(6,60);
      u8g.print("http://endurancelasers.com"); 
      END_SCREEN();
    }
    ...
    void lcd_info_menu() {
      START_MENU();
      MENU_BACK(MSG_MAIN);
      MENU_ITEM(submenu, "Firmware", lcd_info_printer_menu);                   // firmware Info >
      MENU_ITEM(submenu, "Laser", lcd_info_laser_menu);                        // laser Info >
